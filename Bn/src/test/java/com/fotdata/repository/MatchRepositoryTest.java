...
class MatchRepositoryTest {
    ...
    @Test
    void 리그와_라운드로_경기를_조회한다() {
        League league = leagueRepository.save(new League("PL", "Premier League"));
        Team teamA = teamRepository.save(new Team("Arsenal", league, "crest.png"));
        Team teamB = teamRepository.save(new Team("Chelsea", league, "crest2.png"));

        Match match = new Match(league, teamA, teamB, LocalDateTime.of(2025, 9, 1, 0, 0), MatchStatus.SCHEDULED, 3);
        matchRepository.save(match);

        List<Match> result = matchRepository.findByLeagueIdAndMatchday(league.getId(), 3);
